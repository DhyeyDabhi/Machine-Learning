...
def calc_yhat(params):
    ...
    d1 = (np.random.rand(z1.shape[0],z1.shape[1])<keep_prob[0])
    a1 = np.multiply(relu_activation(z1),d1)
    z2 = np.matmul(params[2],a1) + params[3]
    d2 = (np.random.rand(z2.shape[0],z2.shape[1])<keep_prob[1])
    a2 = np.multiply(relu_activation(z2),d2)
    z3 = np.matmul(params[4],a2) + params[5]
    d3 = (np.random.rand(z3.shape[0],z3.shape[1])<keep_prob[2])
    a3 = np.multiply(sigmoid_activation(z3),d3)
    return (z1,a1,z2,a2,z3,a3)

def calc_yhat_test(params):
    #calculates the yhat using forward propogation, given parameters of neural network w(i),b(i)
    #w(i).shape =  (n(i),n(i-1))
    #b(i).shape = (n(i),1)
    #the function returns the tuple of output values i.e z's and a's for future use
...
if __name__=="__main__":
    ...
    w1 = np.random.randn(5,x.shape[0])/10
    b1 = np.zeros((5,1))
    w2 = np.random.randn(5,5)/10
    b2 = np.zeros((5,1))
    w3 = np.random.randn(1,5)/10
    b3 = np.zeros((1,1))
    ...
    x = x.reshape((1,x.shape[0]))
    yhat = calc_yhat_test(prev_params)
